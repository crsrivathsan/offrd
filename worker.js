...
// ── Users who are exempt from the search limit ───────────────────────
...
async function getSearchCount(userId, env) {
  const res = await fetch(
    `${env.SUPABASE_URL}/rest/v1/profiles?id=eq.${userId}&select=search_count`,
    {
      headers: {
...
// Atomically increments search_count via the increment_search_count() RPC
// (see accompanying SQL) — avoids a read-then-write race between two
// requests from the same user landing at the same time.
async function incrementSearchCount(userId, env) {
  const res = await fetch(`${env.SUPABASE_URL}/rest/v1/rpc/increment_search_count`, {
    method: 'POST',
    headers: {
      'Authorization': `Bearer ${env.SUPABASE_SERVICE_KEY}`,
      'apikey': env.SUPABASE_SERVICE_KEY,
  ...
  if (!res.ok) {
    ...
    throw new Error(`Supabase increment failed: ${res.status} ${errText}`);
  }
}

// Returns { allowed, remaining, unlimited, user, reason } — does NOT
// increment. Call this before running the search; call recordSearchUsed()
// only after the search actually succeeds.
async function checkSearchLimit(request, env) {
  const user = await getAuthedUser(request, env);
  if (!user) {
    // No valid session — don't silently let anonymous traffic bypass
    // the limit entirely.
    return { allowed: false, remaining: 0, unlimited: false, reason: 'not_signed_in' };
  }
  if (UNLIMITED_EMAILS.has(user.email)) {
    return { allowed: true, remaining: Infinity, unlimited: true, user };
  }
  const used = await getSearchCount(user.id, env);
  const remaining = Math.max(0, FREE_SEARCH_LIMIT - used);
  return { allowed: remaining > 0, remaining, unlimited: false, user };
}
...
// ════════════════════════════════════════════════════════════════════
// ROUTE: POST /api/search-start — consume one search credit
// Call this ONCE per user-initiated search, before fetching any boards.
// The actual per-board /api/search calls that follow are not separately
// limited — the credit was already spent here.
// ════════════════════════════════════════════════════════════════════
async function handleSearchStart(request, env, ip) {
  if (!checkRateLimit(`search_start_${ip}`, 15)) {
    return errorResponse('Too many requests. Please try again in a minute.', 429);
  }

  const status = await checkSearchLimit(request, env);
  if (!status.allowed) {
    if (status.reason === 'not_signed_in') {
      return errorResponse('Please sign in to search for jobs.', 401);
    }
    return corsResponse({ allowed: false, remaining: 0, limit: FREE_SEARCH_LIMIT }, 200);
  }

  if (!status.unlimited) {
    try {
      await incrementSearchCount(status.user.id, env);
    } catch (err) {
      console.error('[search-start] Failed to increment count:', err);
      return errorResponse('Could not start search. Please try again.', 500);
    }
  }

  const remaining = status.unlimited ? null : Math.max(0, status.remaining - 1);
  return corsResponse({ allowed: true, remaining, limit: FREE_SEARCH_LIMIT, unlimited: status.unlimited });
}

// ════════════════════════════════════════════════════════════════════
// ROUTE: GET /api/search-limit — read remaining searches without using one
// ════════════════════════════════════════════════════════════════════
async function handleSearchLimitStatus(request, env, ip) {
  if (!checkRateLimit(`search_limit_${ip}`, 30)) {
    return errorResponse('Too many requests. Please try again in a minute.', 429);
  }
  const status = await checkSearchLimit(request, env);
  if (status.reason === 'not_signed_in') {
...
async function handleSearch(request, env, ip) {
  ...
  if (source === 'jsearch') {
    ...
    jsearchUrl.searchParams.set('experience_required', params.get('experience') || '');
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (path === '/api/search-start' && request.method === 'POST') return handleSearchStart(request, env, ip);
      if (path === '/api/search-limit' && request.method === 'GET')  return handleSearchLimitStatus(request, env, ip);
