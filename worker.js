...
async function handleTailor(request, env, ip) {
  ...
  const body = await request.json();
  const { messages, system, max_tokens = 16000 } = body;
...
async function handleSearch(request, env, ip) {
  ...
  const requestedNum = Math.max(parseInt(params.get('num') || '10'), 1);
  // JSearch returns at most 10 results per page regardless of what's
  // sent here — anything beyond that needs num_pages. Adzuna uses
  // results_per_page instead and can return more, but the app now
  // requests a fixed 10 per board per page (true pagination via
  // Next/Back) instead of a bigger one-shot batch, so both providers
  // converge on the same per-page size in practice.
  const num      = source === 'adzuna' ? Math.min(requestedNum, 20) : Math.min(requestedNum, 10);
  const numPages = Math.min(Math.max(parseInt(params.get('num_pages') || '1'), 1), 3);
  const page     = params.get('page') || '1';
  const country  = params.get('country') || 'in';
  const where    = params.get('where') || ''; // Adzuna-specific
  ...
  if (source === 'jsearch') {
    ...
    jsearchUrl.searchParams.set('num_pages', String(numPages));
    ...
    // `country` was being parsed above (used for Adzuna) but never
    // actually forwarded to JSearch itself — JSearch silently defaulted
    // to 'us' regardless of what country the person actually selected
    // in the app, which would skew results toward US listings even for
    // an India-targeted search. JSearch's own docs confirm country is a
    // real, supported ISO-code parameter.
    jsearchUrl.searchParams.set('country', country);
  ...
  if (source === 'adzuna') {
    // Adzuna paginates via the path segment (/search/{page}), 1-indexed.
    // This used to be hardcoded to page 1 regardless of what the
    // frontend asked for, which made true Next/Back pagination
    // impossible — clicking Next just re-fetched the same page 1 again.
    const adzunaPage = Math.max(parseInt(page) || 1, 1);
    const adzunaUrl = new URL(
      `https://api.adzuna.com/v1/api/jobs/${country}/search/${adzunaPage}`
    );
    ...
    // Extract redirect_url from each job for direct portal access
    // (bypass Adzuna's intermediary landing page).
