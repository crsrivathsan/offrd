...
const S = {
  ...
  tailoredResumes: {},
  selectedLocations: [],
  // Tracks the current page (1-indexed) for the active search, plus a
  // signature of the search criteria it belongs to. "Remember last page
  // per search" means: re-running the exact same keywords+location+
  // boards+country resumes where the person left off; any change to
  // those starts a fresh search at page 1.
  searchPagination: { signature: '', page: 1, hasMore: true },
  // Which results tab is showing on results.html — 'jsearch' or
  // 'adzuna'. Persisted so navigating away and back keeps the same
  // tab open rather than always resetting to the first one.
  activeResultsTab: 'jsearch'
};
...
const HANDOFF_KEYS = [
  ...
  'searchPagination', 'activeResultsTab'
];
...
// The "Getting Started" checklist is a first-run tutorial, not a
// persistent achievement tracker — once someone has genuinely
// completed it once, it should disappear and stay gone, rather than
// being re-derived every page load from data (search history, pipeline
// items) that can be true for old, unrelated reasons. Concretely: a
// person who searched for a job last month, then comes back today
// with a brand new resume they haven't parsed yet, would otherwise see
// "Search boards" already checked off before they've done anything
// this visit — which is exactly the confusing, contradictory state
// this rewrite avoids.
function updateOnboarding() {
  // Already finished once before — stay hidden permanently, don't
  // recompute or flicker it back into view.
  if (localStorage.getItem('offrd_onboarding_done') === 'true') {
  ...
  const hasSearch = hasResume && !!(S.jobs && S.jobs.length);
  ...
  items.forEach(item => {
    ...
    if (!el) return;
    if (item.done) { el.classList.add('done'); el.classList.remove('active'); const c = el.querySelector('.onboard-check'); if (c) c.textContent = '✓'; }
    else { allDone = false; el.classList.remove('done'); const c = el.querySelector('.onboard-check'); if (c) c.textContent = item.id.slice(-1); if (!el.classList.contains('done')) el.classList.add('active'); }
  });
  const wrap = document.getElementById('onboardWrap');
  if (allDone) {
    localStorage.setItem('offrd_onboarding_done', 'true');
    if (wrap) wrap.style.display = 'none';
  }
}
...
// ─── Blocking Generation Overlay ─────────────────────────────────────
// Used whenever an AI generation step (resume tailoring, cover letter,
// etc.) is running. Previously the progress bar only rendered inside
// the side panel — on mobile, where the panel stacks BELOW the job
// list instead of beside it, that meant the only sign anything was
// happening could be scrolled out of view entirely, so it looked like
// nothing was responding. This puts the same progress UI front and
// center on a full-screen overlay instead, and blocks all interaction
// with the rest of the page until the generation finishes.
//
// The close button (✕) is always available from the start — there's
// no assumption baked in here about how long is "too long." Tailoring
// 5 full resume variants genuinely can take more than a minute, and a
// "this is probably stuck" message at a fixed time threshold turned
// out to fire while it was still working normally and about to
// succeed, which is actively misleading rather than helpful. If
// someone wants out before it finishes, the close button covers that
// without us guessing whether it's actually stuck.

function showGenerationOverlay(title, stages, activeIdx) {
  ...
  if (!overlay) {
    ...
    overlay.innerHTML = `
      <div class="gen-overlay-card">
        <button class="gen-overlay-close" id="genOverlayClose" title="Close" aria-label="Close">✕</button>
        <div class="gen-overlay-title" id="genOverlayTitle"></div>
        <div id="genOverlayProgress"></div>
        <div class="gen-overlay-hint" id="genOverlayHint">This usually takes under a minute — please don't close this tab.</div>
      </div>`;
    document.body.appendChild(overlay);
    document.getElementById('genOverlayClose').onclick = hideGenerationOverlay;
  }
  document.getElementById('genOverlayTitle').textContent = title;
  const hint = document.getElementById('genOverlayHint');
  if (hint) hint.textContent = "This usually takes under a minute — please don't close this tab.";
  overlay.classList.add('open');
  document.body.classList.add('gen-lock'); // disables scroll while overlay is open
  showProgress('genOverlayProgress', stages, activeIdx);
}

function updateGenerationOverlay(stages, activeIdx) {
  if (!document.getElementById('genOverlay')?.classList.contains('open')) return;
  showProgress('genOverlayProgress', stages, activeIdx);
}
...
function hideGenerationOverlay() {
  ...
  document.body.classList.remove('gen-lock');
  hideProgress('genOverlayProgress');
}
