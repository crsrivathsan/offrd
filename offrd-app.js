...
let _genOverlaySafetyTimer = null;
...
function showGenerationOverlay(title, stages, activeIdx) {
  ...
  // After an unusually long wait, soften the "don't close this tab"
  // framing — the close button above is always available regardless,
  // this just lets the person know waiting longer is probably futile.
  if (_genOverlaySafetyTimer) clearTimeout(_genOverlaySafetyTimer);
  _genOverlaySafetyTimer = setTimeout(() => {
    const hintEl = document.getElementById('genOverlayHint');
    if (hintEl) hintEl.textContent = "This is taking longer than expected. You can close this and try again.";
  }, 45000);
}
...
function hideGenerationOverlay() {
  ...
  if (_genOverlaySafetyTimer) { clearTimeout(_genOverlaySafetyTimer); _genOverlaySafetyTimer = null; }
}
