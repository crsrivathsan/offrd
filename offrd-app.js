...
function showGenerationOverlay(title, stages, activeIdx) {
  ...
  // Safety net: if something hangs well beyond a normal generation time
  // (slow connection, a request that never resolves), don't trap the
  // person behind a blocking overlay forever with no way out. Release
  // it automatically and let them try again, rather than the page
  ...
  _genOverlaySafetyTimer = setTimeout(() => {
    ...
    if (hintEl) hintEl.textContent = "This is taking longer than expected. You can wait, or close this and try again.";
    const overlayEl = document.getElementById('genOverlay');
    if (overlayEl && !overlayEl.querySelector('.gen-overlay-dismiss')) {
      const dismissBtn = document.createElement('button');
      dismissBtn.className = 'btn-ghost gen-overlay-dismiss';
      dismissBtn.style.marginTop = '12px';
      dismissBtn.textContent = 'Close and try again';
      dismissBtn.onclick = hideGenerationOverlay;
      overlayEl.querySelector('.gen-overlay-card').appendChild(dismissBtn);
    }
  }, 45000); // 45s — comfortably past the "usually under a minute" framing, before offering an out
}
...
function hideGenerationOverlay() {
  ...
  if (overlay) {
    ...
    overlay.querySelector('.gen-overlay-dismiss')?.remove();
  }
